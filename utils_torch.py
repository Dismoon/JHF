...
def todofp (img):
    ...
    return B
def toBGR (img):
    B=torch.zeros(img.shape[0],4,img.shape[2],img.shape[3])
    B[:,0,:, :]=img[:,2,:,:]
    B[:,1,:, :]=img[:,1,:,:]
    B[:,2,:, :]=img[:,0,:,:]
    B[:, 3, :, :] = img[:, 3, :, :]
    ...
    return B
def save_img(img, filename, j, epoch, data,file='./cyclegan_2/results/RDN/'):
    toPIL = transforms.ToPILImage()
    img.squeeze_(0)
    img = toPIL(img)
    img.save(file+ data +'/%s/'%epoch + '%s_' % j+filename+'.png')

    return True

def save_img2(img,filename,j,category,file='./poldata/test_output/'):
    toPIL = transforms.ToPILImage()
    img.squeeze_(0)
    img = toPIL(img)
    img.save(file+category+'/' + '%s_' % j+filename+'.png')
